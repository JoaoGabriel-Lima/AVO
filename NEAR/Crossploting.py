...
plt.plot(bin_centers, bin_means, color='lime', label='Mean Difference Curve', linewidth=3)
plt.plot(bin_means_vertical, bin_centers_vertical, color='darkred', label='Mean Difference Curve', linewidth=3)
plt.scatter((near_amplitude_scaled), (gradienteFarMinusNear_scaled), c='blue', marker='o')

# Escolher um ponto azul qualquer (exemplo: o primeiro ponto)
ponto_azul = (near_amplitude_scaled[159], gradienteFarMinusNear_scaled[159])

# find near_amplitude_scaled index where value -2.826853*1000
data = np.nonzero(near_amplitude == -2.116889)
print(near_amplitude_scaled[4316])
print(data)
...
def calcular_distancia_horizontal(xs, ys, ponto):
    # Separar as coordenadas do ponto
    x_ponto, y_ponto = ponto
    
    # Calcular as diferenças horizontais entre o ponto e os x's do gráfico
    deltas_x = np.abs(xs - x_ponto)
    
    # Encontrar o índice do x mais próximo ao ponto
    indice_mais_proximo = np.argmin(deltas_x)
    
    # Retornar a distância horizontal
    distancia_horizontal = deltas_x[indice_mais_proximo]
    print(distancia_horizontal)
    plt.scatter(xs[indice_mais_proximo], ys[indice_mais_proximo], c='orange', marker='o', s=100)
    return distancia_horizontal


calcular_distancia_horizontal(bin_centers_vertical, bin_means_vertical, ponto_azul)



x1,y1 = criar_reta_avogd_avoid(ponto_azul)
x2, y2 = criar_reta_avogd_avoid(ponto_azul2)

slope = (y2 - y1) / (x2 - x1)
ax.axline((x1, y1), slope=slope, color="black", linewidth=4)


# ! CRIAR RETA ORTOGONAL A MÉDIA DOS GRADIENTES e INTERCEPTS

plt.xlabel('Near Amplitude')
plt.ylabel('Difference Far and Near')
plt.title('Plot 2D: Crossploting')
