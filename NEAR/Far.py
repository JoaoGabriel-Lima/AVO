...
far_x = [coord[0] for coord in far_coords]  # Usando x das coordenadas far
far_y = [coord[1] for coord in far_coords]    # Usando x das coordenadas Far (pode mudar para y ou z se quiser)
far_z = np.array([coord[2] for coord in far_coords] )   # Usando x das coordenadas Far (pode mudar para y ou z se quiser)

# remove all lines in far_coors where z is -1.e_32
far_coords_NEW = np.array([coord for coord in far_coords if coord[2] != -1.000000e+32])
far_z_scaled = far_z * -10


print(far_coords_NEW)
...
plt.scatter(far_coords_NEW_x, far_coords_NEW_y, c=far_coords_NEW_z, cmap='viridis', marker='o')
